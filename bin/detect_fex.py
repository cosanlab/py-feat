...
if __name__ == '__main__':
    ...
    parser.add_argument("-o", "--outputFname", help="str, Path to output file.", type=str)
    parser.add_argument("-s", "--skip_frames", help="int, Process every n-th frame.", type=int, default=1)
    parser.add_argument("-n", "--n_jobs", help="int, Number of processors to use", type=int, default = 1)
    parser.add_argument("-v", "--verbose", help="int, Number of processors to use", default = False)
...
landmark_columns = np.array([(f'x_{i}',f'y_{i}') for i in range(68)]).reshape(1,136)[0].tolist()
...
frame_columns = ["frame"]
init_df = pd.DataFrame(columns=frame_columns+emotion_columns+facebox_columns+landmark_columns)
if args.outputFname:
...
def process_frame(frame, counter, input_image_size = (48,48), mapper=mapper):
    ...
    try:
        ...
        landmarks_df = pd.DataFrame(landmarks[0][0].reshape(1, 136), columns = landmark_columns, index=[counter])
        # Use Tiankang's colde to align the faces to the center 

        # crop just the face area
        if len(detected_faces)>0:
            facebox_df = pd.DataFrame([detected_faces[0]], columns = facebox_columns, index=[counter])
            ...
            grayscale_cropped_resized_face = grayscale_cropped_face.resize(input_image_size)
            # reshape to put in model
            grayscale_cropped_resized_reshaped_face = np.array(grayscale_cropped_resized_face).reshape(1, img_w, img_h, img_c)
            # normalize
            ...
            emotion_df = pd.DataFrame(predictions, columns = mapper.values(), index=[counter])
            return emotion_df, frame, facebox_df.reindex(index=[counter]), landmarks_df.reindex(index=[counter])
    except:
        return empty_df.reindex(index=[counter]), frame, empty_facebox.reindex(index=[counter]), empty_landmark.reindex(index=[counter])
        ...
        while len(pending_task) > 0 and pending_task[0].ready():
            ...
            if args.inputFname == 0:           
                ...
                if not np.isnan(detected_faces[0]).any():
                    ...
                    labels = [key+": "+str(np.round(df.loc[:, key].values[0],2)) for key in mapper.values()]
                    y0, dy = 50, 30
                    for i, line in enumerate(labels):
                        y = y0 + i*dy
                        cv.putText(frame, line, (30,y), cv.FONT_HERSHEY_SIMPLEX, 1, (0,255,0), 3)
                else:
                    cv.putText(frame, 'No Face Found', (30,30), cv.FONT_HERSHEY_SIMPLEX, 1, (0,255,0), 3)
                # Show preview.
