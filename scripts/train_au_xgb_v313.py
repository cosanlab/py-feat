"""Train xgb_au_v3.13 = v3.4 architecture + expanded BP4D+ corpus (197K rows).

v3.4 ranked tied-1st on number of per-AU wins on DISFA+ (3 each, v3.1 also 3),
hitting AU04=0.601, AU12=0.840, AU20=0.126 — complementary strongholds to
v3.1's AU02/AU17/AU26. The methodology (spatial-split PCA + AUC + in-pipeline
ROS + no subj-norm) is fundamentally different from v3.1's, making it the
natural second ensemble member.

v3.13 retrains v3.4 on the expanded corpus after the BP4D+ padding-bug fix
recovered 47,878 rows (149,960 → 197,838 — matches Cheong's count nearly
exactly). Tests whether v3.4's wins on AU04/12/20 strengthen at scale.

Architecture (unchanged from v3.4):
  - Spatial-split region PCA (Cheong-faithful: zero out off-region HOG cells)
  - ROC-AUC HPO objective
  - In-pipeline RandomOverSampler (always on, fold-local)
  - No per-subject HOG normalization
  - Wider HP space (alpha/lambda 0.1-100, etc.)
  - Broader train_au_dict (keeps BP4D family for AU25/26)
  - early_stopping_rounds=25, n_estimators up to 500

Diff from v3.4:
  - --input-dirs now includes bench-cache/au_train_bp4dplus_v2 (new full BP4D+)
  - load_chunks dedupes by image_path (old BP4D+ chunks in blackwell/3090 are
    proper subsets of the new bp4dplus_v2; dedup keeps the new ones)

Usage:
    python scripts/train_au_xgb_v313.py \\
        --input-dirs bench-cache/au_train_blackwell \\
                     ...
                     bench-cache/au_train_bp4dplus_v2 \\
        --output-dir models/xgb_au_v313 \\
        --device cuda \\
        --n-trials 60
"""
...
# Broader per-AU dataset filter — matches v3.1's set. Cheong's strict version
# (commented below) dropped BP4D/BP4DP from AU25/26 and used CKP-only for AU11;
# on the DISFA+ bench, including more data wins (v3.1 vs v3.2 on AU25/26).
CHEONG_TRAIN_AU_DICT = {
    ...
    "AU11": ["bp4d", "bp4dplus"],  # broader: BP4D+BP4DP (Cheong's "ckplus" had 588 rows)
    ...
    "AU25": ["disfa18", "ckplus", "pain", "bp4d", "bp4dplus", "emotionet"],  # +BP4D vs Cheong
    "AU26": ["disfa18", "ckplus", "pain", "bp4d", "bp4dplus", "emotionet"],  # +BP4D vs Cheong
...
def load_chunks(input_dirs: list[Path]) -> dict:
    """Load + dedupe chunks across input_dirs (keeps first-seen image_path).

    When multiple input_dirs overlap (e.g. old BP4D+ in au_train_blackwell
    AND new full BP4D+ in au_train_bp4dplus_v2), we'd otherwise duplicate
    those rows. Dedup by image_path keeps the result honest.

    Order matters: pass NEW (more complete) dirs FIRST so they win the
    deduplication tie.
    """
    chunks = []
    for d in input_dirs:
    ...
    # Dedup by image_path — keep first occurrence
    paths = np.asarray(merged["image_paths"], dtype=str)
    n_before = len(paths)
    _, first_idx = np.unique(paths, return_index=True)
    first_idx.sort()
    if len(first_idx) < n_before:
        print(f"  deduped {n_before} → {len(first_idx)} ({n_before - len(first_idx)} duplicate paths dropped)")
        merged = {k: v[first_idx] for k, v in merged.items()}
    return merged
...
def train_one_au(
    ...
    def objective(trial):
        params = {
            ...
            "tree_method": "hist",
            "device": device,
            # v3.3-widened HP search: alpha/lambda up to 100 (was 10),
            # min_child up to 40 (was 20), gamma down to 0 (was 1).
            # Cheong's actual picks went up to alpha=70, lambda=42; v3.1
            # had several AUs pinned at alpha=5 boundary.
            "n_estimators": 500,  # large; rely on early stopping
            "max_depth": trial.suggest_int("max_depth", 4, 8),
            "learning_rate": trial.suggest_float("learning_rate", 1e-3, 1.0, log=True),
            "subsample": 0.8,  # fixed like Cheong
            "colsample_bytree": trial.suggest_float("colsample_bytree", 0.8, 1.0),
            "min_child_weight": trial.suggest_int("min_child_weight", 4, 40, step=4),
...
def main():
    ...
    p.add_argument("--output-dir", type=Path, default=Path("models/xgb_au_v313"))
    ...
    # Export v3.13 .skops compatible with feat/au_detectors/.../XGBClassifier
    print("\nexporting v3.13 .skops file...")
    ...
    if missing:
        print(f"  v3.4 missing classifiers for {missing}; falling back to v2")
    ...
    out_skops = args.output_dir / "xgb_au_classifier_v3_13.skops"
