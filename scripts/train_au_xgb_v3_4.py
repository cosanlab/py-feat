"""Train xgb_au_v3.4 = v3.2 design + v3.3's wider HP space + broader train_au_dict.

Bench result rationale (v3.2 = 0.413 mean F1, v3.3 = 0.436, v3.1 = 0.462):
  - v3.2 dominates the "full" region AUs (AU04/05/06/12/20) — the spatial-
    split PCA captures variance well for these
  - v3.3's wider HP space helped AU01/02/09/15/24/28/43 (boundary-pinned in v3.1)
  - Cheong's strict train_au_dict HURT v3.2 on AU25/26 (dropped BP4D family) —
    revert to v3.1's broader dict to keep the BP4D rows

v3.4 = v3.2 architecture + v3.3's wider HP search + broader (non-Cheong) dict.
Same single spatial-split PCA throughout, so .skops is one cohesive bundle.

Diff vs v3.2:
  1. **Wider HP search** (alpha/lambda 0.1-100 log instead of 1-10 log;
     min_child 4-40 instead of 4-20; gamma 0-9 instead of 1-9).
  2. **Broader train_au_dict** — keeps BP4D family for AU25/26 etc., matches
     v3.1's dict. Marked as VFEAT_TRAIN_AU_DICT below.

Diff vs v3.3:
  1. **Spatial-split region PCA** (Cheong-faithful), not row-subset PCA.
  2. **ROC-AUC HPO** (not F1 + leaky threshold sweep).
  3. **In-pipeline RandomOverSampler** (proper per-fold, not trial-level).
  4. **No per-subject HOG normalization.**
  5. **early_stopping_rounds=25 + n_estimators up to 500**, not capped at 200.

Usage:
    python scripts/train_au_xgb_v3_4.py \\
...
def main():
    ...
    p.add_argument("--output-dir", type=Path, default=Path("models/xgb_au_v3_4"))
    ...
    # Export v3.4 .skops compatible with feat/au_detectors/.../XGBClassifier
    print("\nexporting v3.4 .skops file...")
    ...
    out_skops = args.output_dir / "xgb_au_classifier_v3_4.skops"
