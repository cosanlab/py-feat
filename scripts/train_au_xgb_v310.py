"""Train xgb_au_v3.10 = v3.1 architecture (3 PCAs, per-AU routing) with three
Cheong-faithful fixes that v3.1/v3.3 missed:

  1. INTENSITY_BIN_THRESHOLD = 2.0 (was 1.0). Aligns training labels with
     our DISFA+ bench protocol which binarizes truth at >=2. The model no
     longer fires on trace-level intensity-1 evidence that bench scores
     as false positive.
  2. CHEONG_TRAIN_AU_DICT corrected verbatim from Cheong's finetune_xgb.py
     (line 126-146). 7 AUs had wrong dataset filter in v3.1/v3.3:
       AU11 → CKP only (was BP4D+BP4DP)
       AU14/23/24/28 → +CKP added
       AU25/26 → drop BP4D+BP4DP (was including them)
  3. HP space narrowed to Cheong's actual hyperopt ranges (finetune_xgb.py:954):
       n_estimators FIXED at 120 (was 80..300)
       reg_alpha/reg_lambda: 1..10 log (was 0.1..100)
       gamma: 1..9 (was 0..9)
       max_depth: 4..8 (was 4..8 — unchanged)
       min_child_weight: 4..20 step 4 (was 4..40)
       colsample_bytree: 0.8..1 (was 0.7..1)
       learning_rate: 1e-3..1 log (unchanged)
     60 trials (Cheong's max_evals).

Usage:
    python scripts/train_au_xgb_v310.py \\
...
def train_one_au(
    ...
    def objective(trial):
        params = {
            ...
            # (the actual `space={...}` dict Cheong searched over).
...
def main():
    ...
    p.add_argument("--output-dir", type=Path, default=Path("models/xgb_au_v310"))
    ...
    # Step 3: fit 3 region-specific PCAs (Cheong architecture)
    ...
    region_au_masks["full"] = np.array([True] * len(AU_KEYS))  # all AUs
    ...
    # Export v3.10 .skops compatible with feat/au_detectors/.../XGBClassifier
    print("\nexporting v3.10 .skops file...")
    ...
    if missing:
        print(f"  v3.10 missing classifiers for {missing}; falling back to v2")
    ...
    out_skops = args.output_dir / "xgb_au_classifier_v3_10.skops"
    skops_dump(wrapper, out_skops)
    print(f"  wrote {out_skops}")

    # Save per-AU thresholds as a sidecar JSON so the runner can apply them.
