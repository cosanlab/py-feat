"""Train xgb_au_v3.3 = v3.1's winning design + BP4D-male data + Cheong's HP space.

Diff vs v3.1:
  - HP search ranges widened to match Cheong's actual hyperopt ranges
    (see /Storage/Projects/pyfeat_testing/tune_results — alpha up to 70,
    lambda up to 42, lr down to 1e-3). v3.1's narrower ranges hit the
    boundary for several AUs (alpha≈5 for AU15/20/24/26/28/43).
  - Adds bench-cache/au_train_bp4d_male/ (10 BP4D male subjects newly
    extracted, +35K rows).
  - Otherwise identical to v3.1: per-subject HOG normalization, row-subset
    region PCA, F1 HPO with leaky threshold sweep (but threshold isn't
    used at inference, only logged), n_estimators capped at Cheong's 200.

Usage:
    python scripts/train_au_xgb_v3_3.py \\
...
def main():
    ...
    p.add_argument("--output-dir", type=Path, default=Path("models/xgb_au_v3_3"))
    ...
    # Export v3.3 .skops compatible with feat/au_detectors/.../XGBClassifier
    print("\nexporting v3.3 .skops file...")
    ...
    if missing:
        print(f"  v3.3 missing classifiers for {missing}; falling back to v2")
    ...
    out_skops = args.output_dir / "xgb_au_classifier_v3_3.skops"
