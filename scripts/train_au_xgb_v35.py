"""Train xgb_au_v3.5 / v3.6 / v3.7 — ablation on the regional-PCA routing.

The shipped Cheong-inherited design fits 3 PCAs (upper, lower, full) and
each AU classifier only sees ONE of them based on hardcoded anatomical
routing in feat/au_detectors/StatLearning/SL_test.py. This script ablates
that choice — same v3.1-style training otherwise, varying only the
classifier's input feature composition.

Modes (--feature-mode):

  full         (= v3.5)   Fit ONLY a full-face PCA. Every AU classifier
                          uses [full_pca (256) + landmarks (136)] = 392 features.

  upperlower   (= v3.6)   Fit upper + lower PCAs. Every AU classifier uses
                          [upper_pca (256) + lower_pca (256) + landmarks (136)]
                          = 648 features. No full PCA.

  all          (= v3.7)   Fit upper + lower + full PCAs. Every AU classifier uses
                          [upper_pca (256) + lower_pca (256) + full_pca (256)
                           + landmarks (136)] = 904 features.

All three modes use v3.1's row-filtered PCA design (each PCA fit on rows
where its region's AUs are active or all neutral). Compare bench results
to v3.1 (regional routing) to isolate the effect of the routing choice.
