"""Train xgb_au_v3.1: matches Cheong et al. methodology more closely.

Improvements over v3:
  1. Per-subject HOG normalization (subtract subject-mean → expression delta)
  2. Three separate PCA models (upper, lower, full) matching v2 architecture
  3. Per-AU dataset filtering (Cheong's train_au_dict)
  4. XGBoost with `binary:hinge` OR `binary:logistic` (Optuna chooses)
  5. Imblearn RandomOverSampler/UnderSampler for rare AUs
  6. Capped n_estimators (≤200) — Cheong's design choice; avoids overfit
  7. Per-AU threshold sweep on val set (fixes AU20 zero-collapse)
  8. PAIN dataset support for AU20/27/etc.

Input: ``bench-cache/au_train_*/chunk_*.npz`` from
``scripts/extract_au_training_features.py``.

Output: ``models/xgb_au_v3_1/xgb_au_classifier_v3_1.skops`` plus joblib
artifacts.

Usage:
    python scripts/train_au_xgb_v3_1.py \\
        --input-dirs bench-cache/au_train_blackwell \\
                     bench-cache/au_train_3090 \\
                     bench-cache/au_train_pain \\
        --output-dir models/xgb_au_v3_1 \\
        --device cuda \\
        --n-trials 50
"""
...
def train_one_au(
    ...
    def objective(trial):
        params = {
            ...
            "tree_method": "hist",
            "device": device,
            "n_estimators": trial.suggest_int("n_estimators", 80, 200),  # Cheong-style cap
...
def main():
    ...
    p.add_argument("--output-dir", type=Path, default=Path("models/xgb_au_v3_1"))
    ...
    # Export v3.1 .skops compatible with feat/au_detectors/.../XGBClassifier
    print("\nexporting v3.1 .skops file...")
    ...
    if missing:
        print(f"  v3.1 missing classifiers for {missing}; falling back to v2")
    ...
    out_skops = args.output_dir / "xgb_au_classifier_v3_1.skops"
