...
class Fex(DataFrame):
    ...
    def inverse_transform(self, padding_left, padding_top, scale):
        """Helper function to inverse the image transformations applied to the ImageData class for batch processing

        Args:
            padding_left (list, np.array, torch.tensor): amount of left padding applied to each image
            padding_top (list, np.array, torch.tensor): amount of top padding applied to each image
            sca;e (list, np.array, torch.tensor): amount of scaling applied to each image

        Returns:
            Fex Instance
        """

        def convert_to_array(input_variable):
            if isinstance(input_variable, torch.Tensor):
                output_variable = input_variable.numpy()
            else:
                output_variable = np.array(input_variable)
            return output_variable

        data = self.copy()
        padding_left = convert_to_array(padding_left)
        padding_top = convert_to_array(padding_top)
        scale = convert_to_array(scale)

        if (
            len(data) != len(padding_left)
            or len(data) != len(padding_top)
            or len(data) != len(scale)
        ):
            raise ValueError("Data must be the same length as the transformation data.")

        # Remove left side padding & Rescale
        for variable in [
            "FaceRectX",
            "FaceRectWidth",
            "x_0",
            ...
            data[variable] = (data[variable] - padding_left) / scale

        # Remove top side padding & Rescale
        for variable in [
            "FaceRectY",
            "FaceRectHeight",
            "y_0",
            ...
            data[variable] = (data[variable] - padding_top) / scale
