...
class SVMClassifier:
    def __init__(self) -> None:
        self.scaler_upper, self.pca_model_upper = load_classifier_pkl(os.path.join(get_resource_path(), "all_data_Upperscalar_Jun23.pkl")), load_classifier_pkl(os.path.join(get_resource_path(), "all_data_Upperpca_Jun23.pkl"))
        
        self.scaler_lower, self.pca_model_lower = load_classifier_pkl(os.path.join(get_resource_path(), "all_data_Lowerscalar_Jun23.pkl")), load_classifier_pkl(os.path.join(get_resource_path(), "all_data_Lowerpca_Jun23.pkl"))
        
        self.scaler_full, self.pca_model_full = load_classifier_pkl(os.path.join(get_resource_path(), "all_data_Fullscalar_Jun23.pkl")), load_classifier_pkl(os.path.join(get_resource_path(), "all_data_Fullpca_Jun23.pkl"))
        
        self.classifier = load_classifier(
            os.path.join(get_resource_path(), "svm_60_June2023.pkl")
        )
    ...
    def detect_au(self, frame, landmarks):
        ...
        for keys in aus_list:
            if keys in ["AU2", "AU4",  "AU6",  "AU7"]:
                au_pred = self.classifier[keys].predict(pca_transformed_upper)
            elif keys in ["AU10", "AU11", "AU12", "AU14", "AU17", "AU26", "AU28"]:
                au_pred = self.classifier[keys].predict(pca_transformed_lower)
            elif keys in ["AU1", "AU5", "AU9", "AU15",  "AU20", "AU23", "AU24", "AU25", "AU43"]:
    ...
    def detect_au(self, frame, landmarks):
        ...
        for keys in self.au_keys:
            ...
            classifier.load_model(
                os.path.join(get_resource_path(), f"June25_{keys}_XGB.ubj")
            )
        
            if keys in ["AU1","AU2","AU6","AU7","AU43"]:
                au_pred = classifier.predict_proba(pca_transformed_upper)[:, 1]
            elif keys in ["AU10","AU11","AU12","AU14","AU15","AU17","AU23","AU24","AU26","AU28"]:
                au_pred = classifier.predict_proba(pca_transformed_lower)[:, 1]
            elif keys in ["AU4","AU5","AU9","AU20","AU25"]:
