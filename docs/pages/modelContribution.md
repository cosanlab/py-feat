...
3. Your model should be a class that has the appropriate method that a `Detectorv1` can call. For example, Emotion detectors should have the method `mynewmodel.detect_emotions()` that can be called: 
    ...
    ```
4. In `feat/pretrained.py`, add your model to the `PRETRAINED_MODELS` dictionary. If you're adding a new AU detector and it only supports specific AUs make sure to also add it to the `AU_LANDMARK_MAP`. If you're adding an AU detector, you may also want to train a new `PLS` AU→landmark/mesh visualization model; see the model-fitting scripts under `scripts/` in the repo.
