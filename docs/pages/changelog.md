...
- **AU classifier model files migrated to modern format (`*_v2.skops`).** The original `xgb_au_classifier.skops` and `svm_au_classifier.skops` reference their wrapper class as `__main__.{XGB,SVM}Classifier` (a training-script artifact); the xgb file additionally embeds Booster buffers in xgboost's pre-3.x serialization format which the current xgboost C library parses via a deprecated path that intermittently SIGSEGVs on Python 3.13. py-feat 0.7+ downloads the new `xgb_au_classifier_v2.skops` and `svm_au_classifier_v2.skops`; the original files remain on HuggingFace alongside the v2 files for compatibility with py-feat ≤ 0.6.x installs. Both v2 files produce bit-identical predictions to their originals across all 20 sub-classifiers (verified via `scripts/migrate_au_skops.py`).
