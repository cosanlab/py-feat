...
- **`au_to_mesh` v5 AU→mesh model (new default), replacing v4.** `plot_face_mesh` / `predict_face_mesh` / `load_face_mesh_viz_model` now default to `model_version="v5"` ([`py-feat/au_to_mesh`](https://huggingface.co/py-feat/au_to_mesh)) — the 20-AU Detectorv2 **v2.5** space, fit on the v2.5 mesh (CV R² 0.36 vs v4's lower fit) and canonicalized to the same frontal MediaPipe frame as v4. `v4`/`v3`/`v2` remain selectable via `model_version=`.

# 0.7.0

## Notes

The first release since 0.6.2 (March 2024). This is both a major structural overhaul of py-feat's internal `Detectorv1` class and a coordinated batch of breaking changes that bring py-feat onto modern Python and PyTorch. Users on Python 3.10 or earlier, or who use `nltools.data.Adjacency` from `Fex.distance()`, will need to migrate (see the migration guide below).
...
- `Detectorv1.detect_image()` and `Detectorv1.detect_video()` are removed; both flows go through a single `Detectorv1.detect()` with `data_type` in `{'image', 'tensor', 'video'}`.
...
- **`face_model` parameter re-introduced on `Detectorv1`.** The v0.7 changelog originally said face detection was hardcoded to img2pose with no swap; that's no longer accurate. `Detectorv1(face_model='img2pose')` (default) preserves prior behavior with 6DoF head pose. `Detectorv1(face_model='retinaface')` activates a new ResNet34-backbone RetinaFace face detector ported from yakhyo's MIT implementation: 88.9% WIDERFACE Hard AP (vs img2pose's 55.5% per Cheong et al, Affective Science 2023), batched-by-default with `torchvision.ops.batched_nms` on-device, ~210 detections/sec on Apple Silicon at batch 32 versus img2pose's per-image cost. The same `face_model='retinaface'` kwarg works identically on `MPDetector`. With this kwarg, the 6DoF pose columns are populated by a small landmarks-to-pose MLP (`py-feat/pose_mlp_v2`) distilled from img2pose on CelebV-HQ (~5° average MAE vs img2pose); `face_model='img2pose'` returns img2pose's regressed pose directly. Trained weights distributed at `py-feat/retinaface_r34` on HuggingFace (the repo retains the backbone-tagged name; the user-facing kwarg is just `'retinaface'`). Note for v0.7-dev users: an earlier `'retinaface_r34'` spelling that worked only on `Detectorv1` has been dropped — use `'retinaface'` everywhere. *(PRs #275, #277, #285)*
...
- **Memory-efficient identity clustering (new default).** `Fex.compute_identities()` / `cluster_identities()` previously built a dense N×N similarity matrix (connected-components), which OOM'd on long videos (~40 GB at 200K faces). The default is now an incremental "gallery" method (single pass, O(#identities) memory — scales to arbitrarily large inputs) and an `"hdbscan"` option (density-based, emits `"Unknown"` for outliers) was added. **Behavior change:** the gallery default is order-dependent and not transitive, so labels can differ from the old connected-components output; pass `method="connected"` for the exact legacy behavior. Non-finite (no-detection) embeddings are now labelled `NaN`. *Requires scikit-learn ≥ 1.3 for the hdbscan option.*
...
- **`MPDetector` surfaces 20 predicted FACS AUs alongside its 52 MediaPipe blendshapes.** A new linear PLS regressor (`bs_to_au_pls_v2`, [`py-feat/bs_to_au`](https://huggingface.co/py-feat/bs_to_au)) trained on ~350K paired (xgb AU, MP blendshape) frames from 10K CelebV-HQ celebrity videos translates blendshapes → AUs at inference. Closes a long-standing schema mismatch where `MPDetector`'s Fex declared `au_columns` but the actual DataFrame contained the 52 blendshapes. On a DISFA+ benchmark (n=57,150 frames, 12 labeled AUs), the new path beats the standard `Detectorv1` xgb-on-dlib baseline on mean Pearson r (0.51 vs 0.40). *(PRs #300, #302)*
...
- **68-pt dlib → 478-pt MediaPipe mesh bridge.** New `predict_mesh_from_dlib68(landmarks)` reconstructs a 478-vertex 3D mesh from raw `Detectorv1` landmarks via a PCA-bottleneck linear regression (`landmarks68_to_mesh478_pca_v2`, [`py-feat/landmarks68_to_mesh478`](https://huggingface.co/py-feat/landmarks68_to_mesh478)) trained on ~340K paired frames; OOS R² ≈ 0.48. Lets users with a `Detectorv1` Fex render the 3D mesh through `plot_face_mesh(mesh=...)` without running `MPDetector`. *(PR #305)*
...
- **`Detectorv1(device='auto')` / `MPDetector(device='auto')` crash fixed.** When loading the emotion / identity / AU sub-models, `torch.load` was called with `map_location=device` (the raw constructor argument) instead of the resolved `self.device`. With `device='auto'` this passed the literal string `"auto"` to `torch.load`, raising `RuntimeError: don't know how to restore data location ... tagged with auto` — on the exact `device='auto'` path the install docs recommend. All five load sites now use `self.device` (which `set_torch_device` resolves to cuda/mps/cpu). `Detectorv2` was unaffected.
- **`iplot_detections` crash on `Detectorv2` detections fixed.** `Fex.plot_singleframe_detections` (used by `iplot_detections`) hardcoded the v1 emotion names `["anger", "disgust", "fear", "happiness", "sadness", "surprise", "neutral"]` when building each face's emotion annotation, so it raised `KeyError` on `Detectorv2`'s capitalized columns (`Neutral`/`Happy`/...) — even with `emotions=False`, since the annotation is built unconditionally. Now reads `row[self.emotion_columns]`, so interactive plots work for both detectors.

## Documentation updates

- Tutorials updated for the new `Detectorv1.detect()` API.
- New [FAQ](https://py-feat.org/pages/faq.html).

## Migration guide

If you used:
- `Detectorv1.detect_image()` or `Detectorv1.detect_video()` -> change to `Detectorv1.detect(..., data_type='image')` or `data_type='video'`.
...
- you can now pass keyword arguments directly to the underlying pytorch/sklearn models on `Detectorv1` initialization using dictionaries. For example you can do: `detector = Detectorv1(facepose_model_kwargs={'keep_top_k': 500})` to initialize `img2pose` to only use 500 instead of 750 features
...
- `Fex` data-classes returned from `Detectorv1.detect_image()` or `Detectorv1.detect_video()` now store the names of the different detectors used as attributes: `.face_model`, `.au_model`, etc
- The AU visualization model used by `plot_face` and `Detectorv1.plot_detections(faces='aus')` has been updated to include AU11 and remove AU18 making it consistent with Py-feat's custom AU detectors (`svm` and `logistic`)
- A new AU visualization model supporting the `jaanet` AU detector, which only has 12 AUs, has now been added and will automatically be used if `Detectorv1(au_model='jaanet')`.
...
- `Detectorv1` no longer support uninitialized models, e.g. `any_model = None`
  - This is is also true for `Detectorv1.change_model`
